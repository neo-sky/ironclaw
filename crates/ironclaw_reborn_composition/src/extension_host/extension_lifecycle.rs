...
impl RebornLocalExtensionManagementPort {
    ...
    /// Install a caller-supplied package (the IronHub catalog path). `force`
    /// replaces an occupied id in place with rollback on failure; without it
    /// an occupied id is rejected rather than joining the member set.
    pub(crate) async fn install_available_package(
        ...
        self.ensure_not_installed(&available.package.id, &installation_id)
            .await?;
        self.install_fresh_locked(available, caller).await?;
...
"#
    ...
    async fn extension_force_install_materialization_failure_restores_existing_install() {
        ...
        replacement.assets[1].path = "../missing/fixture.wasm".to_string();

        let error = port
            .install_available_package(&replacement, true, &lifecycle_owner())
            .await
