...
/// An entry with no declared provenance defaults to `New`, the unverified tier.
...
/// Built by the host so this policy never reads the environment itself.
...
impl IronHubArtifactHosts {
    pub fn new(catalog_host: Option<String>, operator_hosts: Vec<String>) -> Self {
        Self {
            catalog_host,
...
pub fn tool_summary(
    ...
    Ok(LifecycleSearchExtensionSummary {
        summary: LifecycleExtensionSummary {
            ...
            source: LifecycleExtensionSource::Registry,
...
pub fn skill_summary(
    ...
    Ok(LifecycleSkillSummary {
        ...
        version: entry.version.clone(),
        description: format!("{} [{}]", entry.description, entry.provenance.trust_label()),
        source: LifecycleSkillSource::Registry,
        keywords: Vec::new(),
        tags: Vec::new(),
...
mod tests {
    ...
    fn artifact_urls_must_be_https() {
        let hosts = IronHubArtifactHosts::new(Some("hub.ironclaw.com".to_string()), Vec::new());
