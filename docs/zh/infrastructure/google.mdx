...
1. 使用 `ironclaw serve --host 0.0.0.0` 绑定公网接口（端口不是 `3000` 时追加 `--port <端口>`），或在 `config.toml` 中设置 `[webui].listen_host` / `[webui].listen_port`，以便系统服务也能生效
2. 设置 `IRONCLAW_REBORN_WEBUI_TOKEN=<强随机令牌>`
3. 为该网关端口添加防火墙规则，最好限制为受信任的来源 IP

如果通过 SSH 隧道或 VPN 访问，请保持监听地址为默认值（`127.0.0.1`），不要在 GCE 防火墙中开放该端口。
