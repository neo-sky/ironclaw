...
`extension install` 只接受来自 `extension search` 的**扩展 id**，不接受文件路径或 URL。传入 `./my-tool.wasm` 或 `https://` 链接会失败。

若要添加自行构建的包，请在 [Web 界面](/using/webui)的 **Extensions** 中导入打包好的归档；导入后 `extension search` 便能列出它，再按 id 安装。
